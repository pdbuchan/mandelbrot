...
//                        the Mandelbrot set.
//                      - Maps the smooth iteration count cyclically around the
//                        HSV hue wheel at full saturation and value, then
//                        converts the resulting HSV color to RGB.

// Compile: gcc -Wall hsv2rgb.c -o hsv2rgb -lm
// Usage: ./hsv2rgb
...
static const long double CYCLE_RATE = 0.01L;  // Palette cycles per smooth iteration (e.g., 0.01 CYCLE_RATE gives period = 100 iterations)
...
// Function prototypes
int inputtext (char *);
RGB hsv_palette (long double);
RGB hsv_to_rgb (long double, long double, long double);
...
// Map a smooth iteration count to a cyclic HSV color.
//
// The smooth iteration count is scaled by CYCLE_RATE and wrapped into the
// interval [0, 1), where it represents hue. Saturation and value are both
// held at 1, producing a sequence of fully saturated, full-brightness colors:
//
//   red -> yellow -> green -> cyan -> blue -> magenta -> red
//
// The HSV color is then converted to the RGB representation required by the
// bitmap file.
RGB
hsv_palette (long double smooth) {

  long double h;

  // Scale the smooth iteration count and wrap the hue into [0, 1).
  h = smooth * CYCLE_RATE;
  h -= floorl (h);

  return (hsv_to_rgb (h, 1.0L, 1.0L));
}

// Convert an HSV color to 8-bit RGB.
//
// Hue h is normalized to the interval [0, 1), representing one complete turn
// around the color wheel. Saturation s and value v are in the interval [0, 1].
// The hue wheel is divided into six sectors corresponding to the transitions
// between red, yellow, green, cyan, blue, and magenta.
//
// The intermediate values p, q, and t give the RGB channel levels required
// within the selected hue sector. The resulting RGB components are scaled
// from [0, 1] to the 8-bit range [0, 255].
RGB
hsv_to_rgb (long double h, long double s, long double v) {

  int sector;
  long double fraction, p, q, t, r, g, b;
  RGB c;

  sector = (int) (h * 6.0L);
  fraction = h * 6.0L - sector;
  p = v * (1.0L - s);
  q = v * (1.0L - fraction * s);
  t = v * (1.0L - (1.0L - fraction) * s);
  ...
  c.g = (uint8_t) lroundl (g * 255.0L);
  c.b = (uint8_t) lroundl (b * 255.0L);
