...
static const long double PALETTE_PERIOD = 20.0L;  // Number of smooth iterations per complete color cycle.
...
// Function prototypes
int inputtext (char *);
RGB warm_power_palette (long double);
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < width; x++) {
      ...
      if (((r1 * r1) + (i1 * i1)) > 4.0L) {
        ...
        color = warm_power_palette (smooth);
        fputc (color.b, fo);
        fputc (color.g, fo);
        fputc (color.r, fo);
...
RGB
warm_power_palette (long double smooth) {

  RGB c;
  long double t;

  t = 0.5L * (1.0L - cosl (2.0L * acosl (-1.0L) * smooth / PALETTE_PERIOD));

  c.r = (uint8_t) lroundl (255.0L * t);
  c.g = (uint8_t) lroundl (255.0L * t * t);
  c.b = (uint8_t) lroundl (255.0L * t * t * t);
