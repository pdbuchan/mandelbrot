...
//                        count using a cubic Bezier curve through RGB space.
//                      - Traverses the Bezier palette forward and backward so
//                        successive palette cycles join continuously.
//                      - Points that do not escape within maxit are black.

// Compile: gcc -Wall bezier.c -o bezier -lm
// Usage: ./bezier
...
// Function prototypes
int inputtext (char *);
RGB bezier_palette (long double);
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < width; x++) {
      ...
      if (((r1 * r1) + (i1 * i1)) > 4.0L) {
        ...
        color = bezier_palette (smooth * CYCLE_RATE);
        fputc (color.b, fo);
        fputc (color.g, fo);
        fputc (color.r, fo);
...
// Cubic Bezier palette.
//
// Four RGB colors are treated as the control points P0 through P3 of a cubic
// Bezier curve in RGB space:
//
//   C(t) = (1-t)^3 P0 + 3(1-t)^2 t P1 + 3(1-t)t^2 P2 + t^3 P3.
//
// The smooth iteration coordinate is first wrapped into one palette cycle.
// A cosine then maps that cycle from 0 -> 1 -> 0, causing the Bezier curve to
// be traversed forward and backward.  This avoids a discontinuous jump from
// the final Bezier color back to the initial color at the cycle boundary.
RGB
bezier_palette (long double t) {

  long double a, b, c, d, u;
  static const RGB p0 = {0, 2, 0};        // Black
  static const RGB p1 = {0, 70, 210};     // Blue
  static const RGB p2 = {255, 120, 0};    // Orange
  static const RGB p3 = {255, 255, 255};  // White
  RGB color;

  // Wrap into one cycle and convert it to a smooth forward/backward parameter.
  t -= floorl (t);
  t = 0.5L * (1.0L - cosl (2.0L * PI * t));
  ...
  // Evaluate the cubic Bezier curve independently for R, G, and B.
  color.r = (uint8_t) lroundl (a * p0.r + b * p1.r + c * p2.r + d * p3.r);
  color.g = (uint8_t) lroundl (a * p0.g + b * p1.g + c * p2.g + d * p3.g);
  color.b = (uint8_t) lroundl (a * p0.b + b * p1.b + c * p2.b + d * p3.b);
