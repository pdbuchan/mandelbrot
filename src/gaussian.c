...
//                        count using three phase-shifted Gaussian functions,
//                        one for each RGB channel.
//                      - Uses circular distance so the palette repeats without
//                        a discontinuity at the cycle boundary.

// Compile: gcc -Wall gaussian.c -o gaussian -lm
// Usage: ./gaussian
...
static const long double GAUSSIAN_SIGMA = 0.16L;  // Width of each RGB Gaussian peak
...
// Function prototypes
int inputtext (char *);
RGB gaussian_palette (long double);
long double gaussian_channel (long double, long double);
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < width; x++) {
      ...
      if (((r1 * r1) + (i1 * i1)) > 4.0L) {
        ...
        color = gaussian_palette (smooth * CYCLE_RATE);
        fputc (color.b, fo);
        fputc (color.g, fo);
        fputc (color.r, fo);
...
// Gaussian RGB palette.
//
// Red, green, and blue are generated by Gaussian peaks centered one-third of
// a palette cycle apart. Circular distance is used, so a peak near one end
// of the interval continues smoothly through the other end. The overlap of
// the three bells produces a continuously varying cyclic sequence of colors.
RGB
gaussian_palette (long double t) {
  ...
  r = gaussian_channel (t, 0.0L);
  g = gaussian_channel (t, 1.0L / 3.0L);
  b = gaussian_channel (t, 2.0L / 3.0L);
...
// Evaluate one periodic Gaussian color channel.
//
// The distance between t and center is measured around a unit circle rather
// than along a line. Thus t = 0 and t = 1 represent the same palette
// position. GAUSSIAN_SIGMA controls the width and overlap of the peaks.
long double
gaussian_channel (long double t, long double center) {
  ...
  return (expl (-(d * d) / (2.0L * GAUSSIAN_SIGMA * GAUSSIAN_SIGMA)));
}
