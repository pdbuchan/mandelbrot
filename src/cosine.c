...
//                        the Mandelbrot set.
//                      - Determines color via the cosine method.

// Compile: gcc -Wall cosine.c -o cosine -lm
// Usage: ./cosine
...
static const long double PALETTE_PHASE = 0.2L;  // Changes which color corresponds to a particular smooth iteration value.
...
// Function prototypes
int inputtext (char *);
RGB cosine_palette (long double);
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < width; x++) {
      ...
      if (((r1 * r1) + (i1 * i1)) > 4.0L) { 
        ...
        color = cosine_palette ((smooth * CYCLE_RATE) + PALETTE_PHASE);
        fputc (color.b, fo);
        fputc (color.g, fo);
        fputc (color.r, fo);
...
// Cosine palette.
RGB
cosine_palette (long double t) {

  RGB c;

  // These determine the relative phase between the red, green, and blue channels.
  // Default values of 1/3 and 2/3 result in RGB channels separated by 120 deg.
  // i.e., 0 deg, 120 deg, and 240 deg, respectively.
  const long double green_phase = 1.0L / 3.0L;  // Default: 1/3
  const long double blue_phase = 2.0L / 3.0L;  // Default: 2/3

  c.r = (uint8_t) lroundl (255.0L / 2.0L * (1.0L + cosl (2.0L * PI * (t + 0.0L))));
  c.g = (uint8_t) lroundl (255.0L / 2.0L * (1.0L + cosl (2.0L * PI * (t + green_phase))));
  c.b = (uint8_t) lroundl (255.0L / 2.0L * (1.0L + cosl (2.0L * PI * (t + blue_phase))));
